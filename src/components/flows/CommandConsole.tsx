...
export function CommandConsole({ logs, onCommand, show }: CommandConsoleProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-card/90 border-t border-border p-4 shadow-lg backdrop-blur-sm z-10">
      <div className="max-w-5xl mx-auto">
        <h3 className="text-sm font-medium text-muted-foreground mb-2">Command Console</h3>
        <form onSubmit={handleSubmit} className="flex items-center gap-2 mb-2">
          <input
            type="text"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            placeholder="Type a command (e.g., 'play', 'set intensity to 3')..."
